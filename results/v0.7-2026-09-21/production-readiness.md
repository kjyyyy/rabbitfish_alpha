...
make up PROFILE=vllm                        # vLLM (needs an NVIDIA GPU)
...
1. **No Dockerfile for the lab itself.** The services have one; the lab is installed with pip.
   Worth adding if this ever runs on a schedule somewhere other than a laptop.
